...
当前线上只保留三站测试配置,所有页面渲染相关能力关闭,只验证资源缓存链路。

| 站点 | 离线包 | 资源数 | 原始大小 | 端侧占用 | 上限 | 资源来源 |
|---|---:|---:|---:|---:|---:|---|
| K11 香港 | 开启 | 29 | 6365.1 KB | 5794.4 KB | 7000 KB | 首页/店铺/美食真机分析 + 固定资源 |
| 印尼出境卡 | 开启 | 12 | 1579.0 KB | 593.5 KB | 4096 KB | 自动构建,按大资源/慢资源裁剪 |
| Booking.com | 开启 | 9 | 6819.1 KB | 1803.6 KB | 8192 KB | 真机采集后手动导入 |

三站当前离线包端侧占用合计约 `8191.5 KB`,还会叠加运行时缓存、索引文件和后续新增站点占用。全局沙箱预算当前为 `160 MB`,超过后 SDK 会按 LRU 淘汰旧资源。
...
- 原始大小 `6819.1 KB`。
- 端侧压缩落盘 `1803.6 KB`。
- 端侧日志已确认 `remote bundle: total 9, to download 9`。
...
3. 只选静态 JS/CSS/图片/字体,优先 `>=64 KB` 且 `>=3000 ms`。
4. 在后台导入资源。
5. 看离线包端侧占用是否合理。
