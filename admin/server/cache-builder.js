...
function fileNameForPath(u) {
  return u.replace(/^\/_next\/static\//, '').replace(/\//g, '_');
}
...
function cacheFileRank(file) {
  ...
  if (file.startsWith('css_')) return 1;
  if (file.startsWith('chunks_main') || file.startsWith('chunks_webpack') || file.startsWith('chunks_polyfills')) return 2;
  if (file.startsWith('chunks_')) return 3;
  if (file.startsWith('media_')) return 4;
  return 9;
}
...
async function buildServerCache(appCfg) {
  ...
  return { id: appCfg.id, count: manifest.length, builtAt: new Date().toISOString(), mode: 'server-cache' };
}
